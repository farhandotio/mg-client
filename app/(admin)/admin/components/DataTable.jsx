...
export default function DataTable({ columns, data, loading, onEdit, onDelete, onView }) {
  if (loading) {
    return (
      <div className="w-full h-64 bg-card/20 animate-pulse rounded-0 flex items-center justify-center">
  ...
  return (
    <div className="bg-card border border-border rounded-0 overflow-hidden">
                    ...
                    <div className="flex justify-end gap-2">
                      {onView && (
                        <button
                          aria-label="view"
                          onClick={() => onView(item)}
                          className="p-2 hover:bg-blue-500/10 text-blue-400 rounded-0 transition-all"
                        >
                          <Eye size={16} />
                        </button>
                      )}
